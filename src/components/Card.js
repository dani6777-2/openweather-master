...
const ForecastItem = ({ forecast }) => (
    <div className="bg-violet-50 p-4 rounded-lg shadow-sm hover:shadow-md transition-all duration-200 transform hover:-translate-y-1">
        <p className="text-sm font-semibold text-violet-800 mb-2">{forecast.time}</p>
        <div className="flex items-center space-x-2">
            <img src={forecast.icon} alt="Weather icon" className="w-10 h-10" />
            <div>
                <p className="text-lg font-bold text-gray-800">{forecast.temp}°C</p>
                <p className="text-sm text-gray-600 capitalize">{forecast.desc}</p>
            </div>
        </div>
    </div>
);
...
const Card = ({ loadingData, showData, weather, forecast }) => {
    ...
    if (!showData) return <div className="flex items-center justify-center h-64"><h2 className="text-2xl text-gray-600 font-medium">Sin datos disponibles</h2></div>;
    ...
    // Tomamos los próximos 6 pronósticos (24 horas)
    const forecastData = forecast.list.slice(0, 12).map(item => ({
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="relative overflow-hidden group">
                            ...
                            <div className="absolute inset-0 z-20 p-6 flex flex-col justify-center items-center text-white">
                                ...
                                <p className="text-lg mb-4">{new Date().toLocaleDateString()}</p>
                                <div className="flex items-center justify-center space-x-4">
                                    <span className="text-6xl font-bold">{kelvinToCelsius(weather.main.temp)}°C</span>
                                    <img src={`${baseIconUrl}${weather.weather[0].icon}.png`} alt="Weather icon" className="w-16 h-16" />
                                </div>
                        ...
                        <div className="p-6">
                            <div className="bg-gray-50 rounded-lg p-6 shadow-inner mb-6">
                                <h3 className="text-2xl font-bold mb-6 text-gray-800">Detalles del Clima</h3>
                                <WeatherDetails details={weatherDetails} />
                            </div>

                            <div className="space-y-4">
                                <h3 className="text-xl font-bold text-gray-800 mb-4">Pronóstico 24h</h3>
